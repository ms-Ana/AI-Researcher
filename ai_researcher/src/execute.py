...
cache_dir = (
    "../cache_results_claude_may/execution/factuality_prompting_method_prompting/"
)
...
for python_program_path in tqdm(filenames):
    ...
    with open(python_program_path.replace(".py", "_log.txt"), "w") as log_file:
        ...
        result = subprocess.run(
            ["python3", python_program_path], stdout=log_file, stderr=subprocess.STDOUT
        )
...
print(
    "execution success rate: {} / {} = {}%".format(
        success_count, total_files, success_count / total_files * 100
    )
)
