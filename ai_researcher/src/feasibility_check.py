...
def feasibility_check(experiment_plan, criteria, openai_client, model, seed):
    ...
    prompt += (
        "Here is a list of criteria that would make a project infeasible:\n"
        + criteria.strip()
        + "\n\n"
    )
...
if __name__ == "__main__":
    ...
    for filename in tqdm(filenames):
        ...
        prompt, response, cost = feasibility_check(
            idea, criteria, openai_client, args.engine, args.seed
        )
        print(prompt + "\n")
        print(response + "\n")
        print(cost)
