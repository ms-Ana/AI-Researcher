...
def self_improve(experiment_plan, paper_bank, openai_client, model, seed):
    ...
    prompt += (
        "The original project proposal is:\n"
        + format_plan_json(experiment_plan)
        + ".\n"
    )
    prompt += (
        "Here are a set of related papers provided for you:\n"
...
if __name__ == "__main__":
    ...
    for filename in tqdm(filenames):
        ...
        if (
            "novelty_improvement_papers" in ideas
            and "improved_experiment_plan" in ideas
        ):
        ...
        if args.load_papers_from_cache:
            with open(
                "cache_results/novelty_check/"
                + args.cache_name
                + "_"
                + "_".join(args.idea_name.lower().split())
                + ".json",
                "r",
            ) as f:
        ...
        ## use gpt4 to improve the original experiment plan with the new set of retrieved papers
        print(
            "Improving the original experiment plan with the new set of retrieved papers..."
        )
        prompt, response, cost = self_improve(
            idea, paper_bank[:10], openai_client, args.engine, args.seed
        )
        # print (prompt + "\n")
        # print (response + "\n")
        # print (cost)
