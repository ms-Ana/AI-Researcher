...
import json
import os
from ai_researcher.src.utils import clean_code_output
import retry
...
def execution_generation_method(
    experiment_plan,
    ...
    prompt += (
        "Now write the code to implement the following idea:\n"
        + str(experiment_plan)
        + "\n"
    )
...
if __name__ == "__main__":
    ...
    if args.idea_name == "all":
        filenames = os.listdir(
            os.path.join(cache_dir, "experiment_plans", args.cache_name)
        )
    else:
    ...
    for filename in tqdm(filenames):
        try:
            ...
            if ideas["novelty"] == "yes":
                prompt, response, cost = execution_generation_method(
                    experiment_plan,
                    demo_experiment_plan,
                    demo_execution_code,
                    client,
                ...
                if not os.path.exists(
                    os.path.join(cache_dir, "execution", args.cache_name)
                ):
                    os.makedirs(os.path.join(cache_dir, "execution", args.cache_name))
                with open(
                    cache_file.replace("experiment_plans", "execution").replace(
                        ".json", ".py"
                    ),
                    "w",
                ) as f:
