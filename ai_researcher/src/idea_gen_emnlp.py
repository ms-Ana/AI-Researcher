...
import anthropic
from ai_researcher.src.utils import call_api, shuffle_dict_and_convert_to_string
import argparse
import json
import os
...
def idea_generation(
    existing_ideas, examples, ideas_n, topic_description, openai_client, model, seed
):
    prompt = (
        "You are an expert researcher in Natural Language Processing. Now I want you to help me brainstorm some new research project ideas on the topic of: "
        + topic_description
        + ".\n\n"
    )
    prompt += "You should generate {} different ideas on this topic. Try to be creative and diverse in the idea generation, and do not repeat any similar ideas. ".format(
        str(ideas_n)
    )
