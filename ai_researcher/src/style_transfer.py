...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cache_dir",
        type=str,
        default="../Human_Ideas_Txt",
        help="name of the specific cache dir",
    )
    parser.add_argument("--format", type=str, default="txt", help="either json or txt")
    parser.add_argument(
        "--idea_name", type=str, default="all", help="name of the specific idea file"
    )
    parser.add_argument(
        "--processed_cache_dir",
        type=str,
        default="../Human_Ideas_Txt_Processed",
        help="name of the cache dir to store the processed idea files",
    )
    parser.add_argument(
    ...
    for filename in tqdm(filenames):
        ...
        if args.format == "txt":
            ...
            with open(os.path.join(args.cache_dir, filename), "r") as f:
                human_idea = format_plan_json(
                    json.load(f)["full_experiment_plan"],
                    indent_level=0,
                    skip_test_cases=False,
                    skip_fallback=False,
                )
        prompt, response, cost = style_transfer(
            machine_idea, human_idea, client, args.engine, args.seed
        )
        all_costs += cost
        ...
        with open(
            os.path.join(args.processed_cache_dir, filename.replace(".json", ".txt")),
            "w",
        ) as f:
            f.write(response)
