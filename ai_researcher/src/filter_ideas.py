...
def all_checks(
    ...
    if consistency_check:
        ...
        consistency_prompt, consistency_response, consistency_cost = consistency_score(
            experiment_plan, client, model, seed
        )
    ...
    if feasibility_check:
        ...
        feasibility_prompt, feasibility_response, feasibility_cost = feasibility_score(
            experiment_plan, client, model, seed
        )
    ...
    if significance_check:
        ...
        significance_prompt, significance_response, significance_cost = (
            significance_score(experiment_plan, client, model, seed)
        )
    ...
    if relevance_check:
        ...
        relevance_prompt, relevance_response, relevance_cost = relevance_score(
            experiment_plan, topic_description, client, model, seed
        )
    ...
    if self_novelty_check:
        ...
        self_novelty_prompt, self_novelty_response, self_novelty_cost = (
            self_novelty_score(experiment_plan, client, model, seed)
        )
    ...
    if retrieve_novelty_check:
        ...
        try:
            ...
            for related_paper in paper_bank[:10]:
                (
                    retrieve_novelty_prompt,
                    retrieve_novelty_response,
                    retrieve_novelty_cost,
                ) = retrieve_novelty_score(
                    experiment_plan, related_paper, client, model, seed
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cache_dir",
        type=str,
        default="uncertainty_prompting_method_prompting",
    ...
    parser.add_argument(
        "--score_file",
        type=str,
        default="uncertainty_score_predictions_swiss_round_5",
        help="score file for reranking ideas",
    )
    parser.add_argument(
        "--passed_cache_dir",
        type=str,
        default="uncertainty_prompting_method_prompting",
        help="cache dir for all passed ideas",
    )
    parser.add_argument(
    ...
    for filename in tqdm(top_ideas):
        ...
        check, paper_bank = all_checks(
            topic_description, experiment_plan, client, args.engine, args.seed
        )
        if check:
