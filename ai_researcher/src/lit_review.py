...
def initial_search(
    ...
    if mode == "topic":
        prompt = (
            "You are a researcher doing literature review on the topic of "
...
def paper_score(
    paper_lst, topic_description, openai_client, model, seed, mode="topic", idea=None
):
    ...
    if mode == "topic":
        prompt += (
            "(1) The paper is directly relevant to the topic of: "
            + topic_description.strip()
            + ". Note that it should be specific to solve the problem of focus, rather than just generic methods. "
        )
    elif mode == "idea":
        prompt += (
            "(1) The paper is directly relevant to the proposed idea: "
            + format_plan_json(idea)
...
def collect_papers(
    ...
    _, query, cost = initial_search(
        topic_description, openai_client, model, seed, mode=mode, idea=idea
    )
    total_cost += cost
    ...
    if paper_lst:
        ...
        paper_lst = [
            ...
            if paper["abstract"] and len(paper["abstract"].split()) > 50
        ]
    ...
    while len(paper_bank) < max_papers and iter < 10:
        ...
        grounding_papers = sorted(data_list, key=lambda x: x["score"], reverse=True)[
            :grounding_k
        ]

        ## generate the next query
        _, new_query, cost = next_query(
            topic_description,
            openai_client,
            model,
            seed,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="topic",
        help="do lit review either based on topic or idea",
    )
    parser.add_argument(
        "--topic_description",
        type=str,
        default="automatic evaluation methods and metrics for text-to-image diffusion models",
        help="one-sentence topic description",
    )
    parser.add_argument(
        "--idea_cache",
        type=str,
        default="cache_results_claude_may/experiment_plans_claude3-5/uncertainty_prompting",
        help="cache file for ideas",
    )
    parser.add_argument(
        "--idea_name",
        type=str,
        default="adaptive_uncertainty_sampling",
        help="idea name",
    )
    parser.add_argument(
        "--max_paper_bank_size",
        type=int,
        default=60,
        help="max number of papers to score and store in the paper bank",
    )
    parser.add_argument(
        "--grounding_k",
        type=int,
        default=10,
        help="how many papers for grounding when generating next queries",
    )
    parser.add_argument(
        "--cache_name",
        type=str,
        help="give a name for the output cache file; leave it None if no need caching",
    )
    parser.add_argument(
        "--print_all",
        action="store_true",
        help="whether to print out the intermediate process",
    )
    ...
    paper_bank, total_cost, all_queries = collect_papers(
        ...
        args.seed,
        args.grounding_k,
        max_papers=args.max_paper_bank_size,
