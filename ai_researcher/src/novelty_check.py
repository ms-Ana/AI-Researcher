...
def paper_scoring(paper_lst, idea, topic_description, openai_client, model, seed):
    ...
    prompt += (
        ...
        + " and it should be related to large language models and NLP broadly.\n"
    )
...
def novelty_check(idea_name, idea, topic_description, openai_client, model, seed):
    ...
    for query in all_queries:
        ...
        ## score each paper
        prompt, response, cost = paper_scoring(
            paper_lst, idea, topic_description, openai_client, model, seed
        )
        total_cost += cost
    ...
    ## novelty score
    prompt, novelty, cost = novelty_score(
        sorted_papers, idea, openai_client, model, seed
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="number of top papers to check for novelty",
    )
    parser.add_argument(
        "--retrieve", action="store_true", help="whether to do the paper retrieval"
    )
    parser.add_argument(
        "--novelty", action="store_true", help="whether to do the novelty check"
    )
    parser.add_argument(
    ...
    for filename in tqdm(filenames):
        ...
        cache_file = os.path.join(
            cache_dir + "experiment_plans/" + args.cache_name + "/" + filename
        )
        ...
        try:
            if args.retrieve:
                ...
                paper_bank, total_cost, all_queries = get_related_works(
                    idea_name, idea, topic_description, client, args.engine, args.seed
                )
                ...
                if not os.path.exists(
                    cache_dir + "experiment_plans/" + args.cache_name + "/"
                ):
            ...
            if args.novelty:
                ...
                for i in range(args.check_n):
                    prompt, response, cost = novelty_score(
                        plan_json, related_papers[i], client, args.engine, args.seed
                    )
    ...
    if args.novelty:
        print(
            "Novelty rate: {} / {} = {}%".format(
                novel_idea, len(filenames), novel_idea / len(filenames) * 100
            )
        )
