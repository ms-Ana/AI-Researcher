...
def plan_generation_method(
    method, idea, demo_examples, topic_description, openai_client, model, seed
):
    ## forumate an idea from a paragraph into a full experiment plan based on our template

    prompt = (
        "You are an expert researcher in Natural Language Processing and your job is to expand a brief project idea into a full project proposal with detailed experiment plans so that your students can follow the steps and execute the full project. I will provide you with an idea on the topic of: "
        + topic_description
        + ".\n\n"
    )
    ...
    prompt += (
        "Below are a few examples of how the full experiment plans should look like:\n"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="dir that stores all the raw ideas",
    )
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="dir to store all the generated experiment plans",
    )
    parser.add_argument(
        "--cache_name",
        type=str,
    ...
    for idea_name in tqdm(idea_names):
        cache_file = os.path.join(
            args.experiment_plan_cache_dir
            + args.cache_name
            + "/"
            + "_".join(idea_name.lower().split())
            + ".json"
        )
        ...
        try:
            ...
            prompt, response, cost = plan_generation_method(
                args.method,
                idea,
                demo_examples,
                topic_description,
