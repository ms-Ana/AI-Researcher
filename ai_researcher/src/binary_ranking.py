...
def self_consistency(
    idea_1, idea_2, method, openai_client, model, seed, sc_n=10, few_shot_demos=None
):
    predictions = []
    costs = 0
    for i in range(sc_n):
        new_method = method.replace("_sc", "")
        prompt, response, cost = better_idea(
            idea_1,
            idea_2,
            new_method,
            openai_client,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="methods: {zero_shot, zero_shot_cot, few_shot, few_shot_cot} x {normal, sc}",
    )
    parser.add_argument(
        "--sc_n", type=int, default=10, help="number of sampling for self-consistency"
    )
    ...
    for i in tqdm(range(N)):
        ...
        if "_sc" not in args.method:
            if label == 1:
                prompt, response, cost = better_idea(
                    pos_idea["structured_summary"],
                    neg_idea["structured_summary"],
                    args.method,
                    client,
                    args.engine,
                    args.seed,
            ...
            if label == 1:
                prompt, response, cost = self_consistency(
                    pos_idea["structured_summary"],
                    neg_idea["structured_summary"],
                    args.method,
                    client,
